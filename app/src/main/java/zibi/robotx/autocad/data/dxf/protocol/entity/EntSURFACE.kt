...
class EntSURFACE(lastElem: DxfChain, isRead: Boolean) : EntCommon() {
	...
	//	71	
	var NumberU: Int? = null

	//	 Number of V isolines
	//	72	
	var NumberV: Int? = null
    ...
    while (true) {
        ...
        if (super.readX(cod)) {
			...
			"71" -> {
				NumberU = getbfr.intValue()
			}
			"72" -> {
				NumberV = getbfr.intValue()
			}
			}
		}
			cod = getbfr.get()
	...
	//	100	
	const val Markar: String = "AcDbSurface"
	 }
	 ...
	 override fun write( sbX: StringBuilder): StringBuilder {
		...
		if( NumberU != null) sb.append( "\n 71\n"+NumberU )
		if( NumberV != null) sb.append( "\n 72\n"+NumberV )
		if( sb.isNotEmpty() ) {
 			sbX.append( "\n 0\nSURFACE")
			sbX.append( sb)
		}
