...
class EntATTDEF(lastElem: DxfChain, isRead: Boolean) : EntCommon() {
	...
	//	 First alignment point (in OCS), DXF: X value APP: 3D pointDXF: Y and Z values of text start point (in OCS)
	//	10	20	30	
	...
	//	 Second alignment point (in OCS) (optional), DXF: X value APP: 3D point, Meaningful only if 72 or 74 group values are nonzeroDXF: Y and Z values of second alignment point (in OCS) (optional)
	//	11	21	31	
    ...
    while (true) {
        ...
        if (super.readX(cod)) {
			...
			"3" -> {
				Prompt = getbfr.stringValue()
			}
			"2" -> {
				TagContain = getbfr.stringValue()
			}
			"70" -> {
	...
	//	100	
	const val AcDbAttributeDefinition: String = "AcDbAttributeDefinition"
